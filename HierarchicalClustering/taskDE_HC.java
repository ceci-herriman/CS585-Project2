...
javac -classpath $(hadoop classpath) taskDE_HC.java
jar cf taskDE_HC.jar taskDE_HC*.class
rm -rf ~/shared_folder/project2/part2/partA/output
hdfs dfs -rm -r -f /user/ds503/project2/part2/partA/output
hadoop jar taskDE_HC.jar taskDE_HC
...
public class taskDE_HC {
    ...
    // custom writable for partial sums + count (for combiner/reducer)
    public static class SumCountWritable implements org.apache.hadoop.io.Writable {
        public double[] sum = new double[4];
        public int count;

        public SumCountWritable() {
        }

        public SumCountWritable(double[] point) {
            for (int i = 0; i < 4; i++) sum[i] = point[i];
            count = 1;
        }

        @Override
        public void write(java.io.DataOutput out) throws java.io.IOException {
            for (int i = 0; i < 4; i++) out.writeDouble(sum[i]);
            out.writeInt(count);
        }

        @Override
        public void readFields(java.io.DataInput in) throws java.io.IOException {
            for (int i = 0; i < 4; i++) sum[i] = in.readDouble();
            count = in.readInt();
        }
    }

    // MAPPER
    public static class HCMapper extends Mapper<Object, Text, Text, SumCountWritable> {
        // use hashmap for centroids (key = centroid string, value = double[])
        java.util.Map<String, double[]> seedsMap = new java.util.HashMap<>();
        ...
        protected void setup(Context context) throws IOException {
            ...
            while ((line = br.readLine()) != null) {
                line = line.trim();
                if (!line.isEmpty()) {
                    double[] c = stringToCentroid(line);
                    seedsMap.put(line, c);
                }
            }
            br.close();
        ...
        public void map(Object key, Text value, Context context)
            ...
            for (java.util.Map.Entry<String, double[]> entry : seedsMap.entrySet()) {
                double distance = euclideanDistance(pointDoubles, entry.getValue());
            ...
            if (closestKey != null)
                context.write(new Text(closestKey), new SumCountWritable(pointDoubles));
        }
    }

    // COMBINER
    public static class HCCombiner extends Reducer<Text, SumCountWritable, Text, SumCountWritable> {
        ...
        public void reduce(Text key, Iterable<SumCountWritable> values, Context context) throws IOException, InterruptedException {
            ...
            for (SumCountWritable val : values) {
                for (int i = 0; i < 4; i++) sum[i] += val.sum[i];
                count += val.count;
            }

            SumCountWritable out = new SumCountWritable();
            out.sum = sum;
            out.count = count;
    ...
    // REDUCER
    public static class HCReducer extends Reducer<Text, SumCountWritable, Text, NullWritable> {
    ...
    public static void main(String[] args)
        ...
        while (centroids.size() > 1) {
            ...
            job.setJarByClass(taskDE_HC.class);

            job.setMapperClass(HCMapper.class);
            job.setCombinerClass(HCCombiner.class);
        ...
        job2.setJarByClass(taskDE_HC.class);

        job2.setMapperClass(HCMapper.class);
        job2.setCombinerClass(HCCombiner.class);
        job2.setReducerClass(HCReducer.class);

        job2.setMapOutputKeyClass(Text.class);
        job2.setMapOutputValueClass(SumCountWritable.class);
